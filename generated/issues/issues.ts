    ...
    .then(
      (promisesSettled) => {
        (promisesSettled.filter(
          promise => promise.status === 'fulfilled') as PromiseFulfilledResult<SourceIssues>[]
        )
          .forEach(({ value }) => {
...
export default function IssuesPagesPlugin({ usedLabels }: IssuesPagesPluginConfig): Plugin {
  return {
    enforce: 'pre',
    ...
    load(id) {
      ...
      if (mapSourceIssues.has(id))
        return mapSourceIssues.get(id)!.content
    },
  }
}
